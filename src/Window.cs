...
class Window {
  ...
  public Gtk.ApplicationWindow _Window // Public property used to access the
                                       // window attribute in read-only
      {
  ...
  public Window(string title, int sizeX, int sizeY, Gio.Application sender) {
    this.window =
        Gtk.ApplicationWindow.New((Gtk.Application)sender); // Create the window
  ...
  public void SetHeaderBar() {
    ...
    header_bar.PackStart(
        Gtk.Label.New("Mettre la menu bar ici")); // We put a label at the
                                                  // beginning of the header bar

    var button = Gtk.MenuButton.New(); // We create a button
    button.SetHasFrame(false);         // without a frame
    var button_icon = Gtk.Image.NewFromGicon(Gio.ThemedIcon.New(
        "open-menu-symbolic")); // We create an image with an icon
    // The names of the available icons can be found with `gtk4-icon-browser`,
    // or in /usr/share/icons/
    button.SetChild(button_icon); // We set the icon as child of the button (the
                                  // child will be contained in the button)

    var pop = Gtk.Popover.New(); // New popover menu
    var box = Gtk.Box.New(Gtk.Orientation.Vertical,
                          0);            // New box to put in the popover menu
    var button_about = Gtk.Button.New(); // Button to put in the box
    button_about.SetLabel(
        Globals.lan.ServeTrad("about")); // Label of the button
    ...
    button_about.OnClicked += (sender, args) => {
      ...
      dialog.Application =
          (Gtk.Application)this
              .sender; // CS0030: Impossible de convertir le type 'Gtk.Button'
                       // en 'Gtk.Application'
      dialog.Show();
    };
  ...
  public Gtk.ScrolledWindow MakePDFViewer() {
    // TODO: Compatibily with Window is broke here (due to the paths being
    // hardcoded) Load the PDF file into IronPdf
    IronPdf.PdfDocument pdf = new IronPdf.PdfDocument("./assets/pdf_test.pdf");

    // Render the PDF as images in temp folder
    pdf.RasterizeToImageFiles("/tmp/*.png", 2160, 3840,
                              IronPdf.Imaging.ImageType.Png, 300);

    var image_box = Gtk.Box.New(Gtk.Orientation.Vertical, 0);

    // TODO: Find another way to show images than converting them to images, it
    // is not convenient
    ...
    for (int i = 1; i <= pdf.PageCount; ++i) {
      ...
      // IMPORTANT : this need to be on 'false' or else, the scrolled window
      // will not work
      imagePdf.SetCanShrink(false);
      // Keep the aspect ratio of the image, which avoid the image to be
      // stretched when resizing the window
