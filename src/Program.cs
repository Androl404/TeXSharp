...
class Program {
  public static int Main(string[] args) {
    var application = Gtk.Application.New("com.github.TeXSharp",
                                          Gio.ApplicationFlags.FlagsNone);
    application.OnActivate += (sender, args) => {
      // Create main Window
      var window = new Window(
          $"TeXSharp - {Globals.lan.ServeTrad("modern_latex_editor")}", 800,
          600, sender);
      window.SetHeaderBar();
      ...
      grid.Attach(PDF_viewer, 2, 2, 3, 1);  // Spans 2 columns in the third row
